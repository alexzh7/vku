...
tippy('#smoke', {
    content: 'Датчик дыма SS800 сигнализирует о задымленности помещений, предупреждая звуковыми сигналами об опасности пожара и отравления угарными газами, присылает уведомления в приложении и запускает пользовательсике сценарии',
    placement: 'bottom'
});

tippy('#plug', {
    content: 'Умная розетка SP800 позволяет управлять любыми потребителями с заявленной мощностью потребления до 2.5 кВ',
});

tippy('#hub', {
    content: 'Центр управления Smart Hub SH800 производит коммутацию и управление всеми устройствами и датчиками умного дома экосистемы VKU по протоколу ZigBee',
    placement: 'left-start'
});

tippy('#multisensor', {
    content: 'Многофункциональный датчик SM800 сочетает в себе датчик температуры и влажности окружающего воздуха, датчик освещенности и датчик движения. Оснащен датчиком демонтажа для оповещения о несанкционированном снятии устройства с крепления.',
    placement: 'bottom'
});

tippy('#water', {
    content: 'Автоматизируйте полив участка с помощью реле SR800 или умных розеток SP800',
    placement: 'top'
});

tippy('#webcam-2', {
    content: 'Видеокамера VKU-G14 обеспечивает безопасность Вашей придомовой территории, имеет ночной режим работы, угол обзора 100°, детектор движения и разрешение Full HD 1080р',
    placement: 'bottom'
});
